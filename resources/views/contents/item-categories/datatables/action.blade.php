@if (auth()->user()->hasAnyPermission(['item-category.edit', 'item-category.destroy']))
    ...
    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-150px py-3"
        ...
        @can('item-category.edit')
            <div class="menu-item px-3">
                <a href="{{ route('item-categories.edit', $query->id) }}" class="menu-link px-3">
                    <i class="ki-solid ki-pencil fs-4 me-2 text-warning"></i>
                    {{ __('Edit') }}
                </a>
            </div>
        @endcan
        @can('item-category.destroy')
            <div class="menu-item px-3">
