...
<x-main-app-layout :title="$title" :breadcrumbs="$breadcrumbs">
    <div id="kt_app_content_container" class="app-container mt-5 mt-lg-9">
        <form id="form" onsubmit="return false" novalidate="novalidate" class="form d-flex flex-column flex-lg-row"
            data-url-action="{{ route('items.update', $query->id) }}">
            @method('PUT')
            ...
            <div class="d-flex flex-column gap-7 gap-lg-10 w-100 w-lg-300px mb-7 me-lg-10">
                <div class="card card-flush">
                    <div class="card-header border-0">
                        ...
                        <div class="card-toolbar">
                            <div id="item_status"
                                class="rounded-circle {{ $query->is_active === 1 ? 'bg-success' : 'bg-secondary' }} w-15px h-15px">
                            </div>
                        </div>
                    </div>
                    <div class="card-body border-top p-9">
                        <select id="status" name="status" class="form-select mb-2" data-control="select2"
                            ...
                            <option value="1" {{ $query->is_active === 1 ? 'selected' : '' }}>Active</option>
                            <option value="0" {{ $query->is_active !== 1 ? 'selected' : '' }}>Inactive</option>
                        </select>
                ...
                <div class="card card-flush">
                    ...
                    <div class="card-body border-top p-9 text-center">
                        ...
                        <div class="image-input {{ $query->thumbnail_url ? '' : 'image-input-empty' }} image-input-outline image-input-placeholder mb-3"
                        ...
                        <select id="categories" name="categories[]" class="form-select mb-2" data-control="select2"
                            ...
                            @forelse ($itemCategories as $item)
                                <option value="{{ $item->id }}"
                                    {{ $query->itemCategories->contains('id', $item->id) ? 'selected' : '' }}>
                                    {{ $item->name }}
                                </option>
                            @empty
                            @endforelse
                    ...
                    <div class="card-body border-top p-9">
                        <div class="mb-10 fv-row">
                            ...
                            <input type="text" id="name" name="name" class="form-control mb-2"
                                placeholder="Item Name" value="{{ $query->name }}" autocomplete="one-time-code" />
                    ...
                    <div class="card-body border-top p-9">
                        <div class="mb-10 fv-row">
                            ...
                            <div class="d-flex flex-column gap-3" data-repeater-list>
                                @forelse ($query->itemDetails as $itemDetail)
                                    <div class="form-group d-flex flex-wrap align-items-center gap-5"
                                        data-id="{{ $loop->iteration }}" data-repeater-item>
                                        <div
                                            class="w-100 w-md-50px form-check form-switch form-check-custom form-check-success form-check-solid">
                                            <input type="checkbox" name="details[{{ $loop->iteration }}][status]"
                                                class="form-check-input w-50px cursor-pointer" value="1"
                                                {{ $itemDetail->is_active ? 'checked' : '' }} />
                                        </div>
                                        <div class="form-floating w-100 w-md-200px">
                                            <select id="detail_{{ $loop->iteration }}_unit_of_measurement"
                                                name="details[{{ $loop->iteration }}][unit_of_measurement]"
                                                class="form-select" data-control="select2" data-hide-search="true"
                                                data-placeholder="Select a variation">
                                                @forelse ($unitOfMeasurements as $item)
                                                    <option value="{{ $item->id }}"
                                                        {{ $item->id === $itemDetail->unit_of_measurement_id ? 'selected' : '' }}>
                                                        {{ $item->name }}
                                                    </option>
                                                @empty
                                                @endforelse
                                            </select>
                                            <label for="detail_{{ $loop->iteration }}_unit_of_measurement">
                                                Unit of Measurement
                                            </label>
                                        </div>
                                        <div class="form-floating">
                                            <input type="text" id="detail_{{ $loop->iteration }}_quantity"
                                                name="details[{{ $loop->iteration }}][quantity]"
                                                class="form-control mw-100 w-200px input-number-integer"
                                                value="<x-number-format :amount='(integer) $itemDetail->quantity' :min-fraction-digits='0' />"
                                                placeholder="Quantity" autocomplete="one-time-code" />
                                            <label for="detail_{{ $loop->iteration }}_quantity">Quantity</label>
                                        </div>
                                        <div class="form-floating">
                                            <input type="text" id="detail_{{ $loop->iteration }}_cost"
                                                name="details[{{ $loop->iteration }}][cost]"
                                                class="form-control mw-100 w-200px input-number-decimal"
                                                value="<x-number-format :amount='(float) $itemDetail->cost' :min-fraction-digits='0' />"
                                                placeholder="Cost" autocomplete="one-time-code" />
                                            <label for="detail_{{ $loop->iteration }}_cost">Cost</label>
                                        </div>
                                ...
                                @empty
                                    <div class="form-group d-flex flex-wrap align-items-center gap-5" data-id="1"
                                        ...
                                        <div class="form-floating w-100 w-md-200px">
                                            <select id="detail_1_unit_of_measurement"
                                                name="details[1][unit_of_measurement]" class="form-select"
                                                data-control="select2" data-hide-search="true"
                                                data-placeholder="Select a variation" data-kt-initialized="1">
                                                <option></option>
                                                @forelse ($unitOfMeasurements as $item)
                                            ...
                                            <label for="detail_1_unit_of_measurement">Unit of Measurement</label>
                                        </div>
                                        <div class="form-floating">
                                            <input type="text" id="detail_1_quantity" name="details[1][quantity]"
                                                class="form-control mw-100 w-200px input-number-integer"
                                                placeholder="Quantity" autocomplete="one-time-code" />
                                            <label for="detail_1_quantity">Quantity</label>
                                        </div>
                                        <div class="form-floating">
                                            <input type="text" id="detail_1_cost" name="details[1][cost]"
                                                class="form-control mw-100 w-200px input-number-decimal"
                                                placeholder="Cost" autocomplete="one-time-code" />
                                            <label for="detail_1_cost">Cost</label>
                                        </div>
                                        <button type="button" class="btn btn-sm btn-light-danger"
                                            button-repeater-remove>
                                            <i class="ki-duotone ki-trash fs-4">
                ...
                <div class="card card-flush">
                    ...
                    <div class="card-body border-top p-9">
                        <div class="mb-10 fv-row">
                            ...
                            <div class="d-flex flex-column gap-3" data-repeater-list>
                                @forelse ($query->itemCodes as $itemCode)
                                    <div class="input-group" data-id="{{ $loop->iteration }}" data-repeater-item>
                                        <div class="form-floating">
                                            <input type="text" id="code_{{ $loop->iteration }}_value"
                                                name="codes[]" class="form-control" value="{{ $itemCode->value }}"
                                                placeholder="Unique Code" autocomplete="one-time-code" />
                                            <label for="code_{{ $loop->iteration }}_value">Unique Code</label>
                                        </div>
